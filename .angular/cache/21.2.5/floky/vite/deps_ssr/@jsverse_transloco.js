    ...
    const load$ = loadTranslation.pipe((0, import_rxjs.retry)(this.config.failedRetries), (0, import_rxjs.tap)((translation) => {
      if (Array.isArray(translation)) {
        translation.forEach((t2) => {
          ...
          if (t2.lang !== path) {
            this.cache.set(t2.lang, (0, import_rxjs.of)({}));
          }
        });
        return;
      }
...
function translateSignal(key, params, lang, injector) {
  ...
  const result = runInInjectionContext(injector, () => {
    ...
    return computerKeysAndParams(key, params).pipe((0, import_rxjs.switchMap)((dynamic) => service2.selectTranslate(dynamic.key, dynamic.params, scope)));
  });
...
function translateObjectSignal(key, params, lang, injector) {
  ...
  const result = runInInjectionContext(injector, () => {
    const service2 = inject(TranslocoService);
    const scope = resolveScope(lang);
    return computerKeysAndParams(key, params).pipe((0, import_rxjs.switchMap)((dynamic) => service2.selectTranslateObject(dynamic.key, dynamic.params, scope)));
  });
  return toSignal(result, {
    initialValue: Array.isArray(key) ? [] : {}
  });
